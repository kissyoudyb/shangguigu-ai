...
class LogisticsLogisticsCategory(str, enum.Enum):
    退货 = '退货'
    换货退货 = '换货退货'
    换货发货 = '换货发货'


class PostsalePostsaleType(str, enum.Enum):
    退款 = '退款'
    退货 = '退货'
    换货 = '换货'
...
class Logistics(Base):
    ...
    logistics_category: Mapped[Optional[LogisticsLogisticsCategory]] = mapped_column(Enum(LogisticsLogisticsCategory, values_callable=lambda cls: [member.value for member in cls]), comment='物流类别')
...
class Postsale(Base):
    ...
    postsale_type: Mapped[Optional[PostsalePostsaleType]] = mapped_column(Enum(PostsalePostsaleType, values_callable=lambda cls: [member.value for member in cls]), comment='售后类型')
